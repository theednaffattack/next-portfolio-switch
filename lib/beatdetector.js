...
(function() {
  ...
  this.BeatDetector = function(settings) {
    ...
    this.visualizer.fftSize = settings.visualizerFFTSize
      ? settings.visualizerFFTSize
      : 256;
    this.analyser.fftSize = settings.analyserFFTSize
      ? settings.analyserFFTSize
      : 256;
    ...
    for (
      i = 0;
      this.historyBuffer.length < this.MAX_COLLECT_SIZE - this.COLLECT_SIZE - 1;
      i++
    ) {
    ...
    this.bandpassFilter.type =
      typeof this.bandpassFilter.type === "string" ? "bandpass" : 2;
    this.bandpassFilter.frequency.value = settings.passFreq
      ? settings.passFreq
      : 400;
  ...
  this.BeatDetector.prototype = {
    ...
    pause: function() {
      ...
      } else if (typeof this.micStream !== "undefined") {
        //or mic
        this.micStream.stop();
      }
    },
    ...
    isFinished: function(offset) {
      ...
      if (this.getElapsedTime() >= dur && dur != 0) {
        //played whole buffer
        if (this.soundSource.playbackState === this.soundSource.PLAYING_STATE) {
          this.soundSource.stop(0);
    ...
    isOnBeat: function() {
      ...
      if (
        instantCounter > this.COLLECT_SIZE - 1 &&
        this.historyBuffer.length > this.MAX_COLLECT_SIZE - 1
      ) {
        this.instantEnergy =
          this.instantEnergy /
          (this.COLLECT_SIZE * (this.analyser.fftSize / 2));
        ...
        if (timeDiff > 2 && this.bpmTable.length > 0) {
          ...
          for (var j = 0; j < this.bpmTable.length - 1; j++) {
            ...
            var timeDiffInteger = Math.round(
              timeDiff / this.bpmTable[j]["time"] * 1000
            );
            ...
            if (
              timeDiffInteger % (Math.round(this.bpmTable[j]["time"]) * 1000) ==
              0
            ) {
        ...
        if (
          ...
          this.instantEnergy > localAverageEnergy &&
      ...
      for (i = 0; i < 10; i++) {
        if (i >= this.bpmTable.length) break;

        this.debug +=
          "Beat " +
          i +
          ": " +
          this.bpmTable[i]["time"] +
          ", counter: " +
          this.bpmTable[i]["counter"] +
          ", calc. bpm: " +
          Math.round(60 / this.bpmTable[i]["time"]) +
          "<br>";
      }

      this.debug +=
        "history buffer size: " + this.historyBuffer.length + "<br>";
      ...
      this.debug +=
        "sensitivity: " + ((this.sens - 1) * 100).toFixed(2) + "<br>";
    ...
    getDuration: function() {
      return typeof this.soundBuffer === "undefined"
        ? 0
